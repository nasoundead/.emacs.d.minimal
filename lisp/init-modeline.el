...
`all-the-icons-octicon' to fetch the icon."
  (concat (if vc-mode " " "  ")
          (when icon
            (concat
             (all-the-icons-material icon :face face :height 1.0 :v-adjust (or voffset -0.225))
    ...
    (when-let ((icon
                (pcase flycheck-last-status-change
                  ('finished (if flycheck-current-errors
                               ...
                               (+sea-ml-icon "check" "-" 'sea-modeline-info)))
                  ('running     (+sea-ml-icon "access_time"    "*" 'font-lock-doc-face))
                  ('no-checker  (+sea-ml-icon "sim_card_alert" "-" 'font-lock-doc-face))
                  ('errored     (+sea-ml-icon "sim_card_alert" "!" 'sea-modeline-urgent))
                  ('interrupted (+sea-ml-icon "pause"          "x" 'font-lock-doc-face)))))
      (propertize
       icon))))
;;
